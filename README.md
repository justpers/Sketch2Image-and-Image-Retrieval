...
- **주요 기능**:
    - **Sketch Captioning**: 손가락 제스처로 그린 스케치를 설명하는 캡션 생성
    - **Diffusion**: 스케치와 스케치 캡션을 입력으로 받아 고해상도 이미지를 생성
    - **Image Retrieval**: 생성된 이미지를 기반으로 갤러리에서 유사한 이미지를 검색
...
**2. BLIP (Bootstrapped Language-Image Pre-training)**:
   ...
   - 생성된 캡션을 Diffusion 모델의 프롬프트로 사용

 초기 BLIP Inference 도출 시, 아래 왼쪽 그림과 같이 'a drawing of ~'로 캡셔닝 되어, 이것을 Diffusion 모델에 넣었을 때 그림체 느낌의 이미지가 생성되었습니다. 하지만 저희 팀은 고품질 이미지가 생성되기를 원했기 때문에, 실험을 통해 고품질 이미지로 변환할 수 있는 문장을 알아내고자 했고, 'Convert a [class] into a high-resolution color image with a white backgroud' 라는 문장을 찾았습니다. 따라서 스케치를 넣었을 때 해당 캡션이 생성되도록 Sketchy Dataset의 Sketch와 그에 대한 캡션으로 데이터셋을 만들어 이것으로 BLIP을 fine tuning하였습니다. 그 결과 스케치에 대한 캡션과 생성된 이미지는 아래 오른쪽 그림과 같이 나왔습니다.
...
**4. 실행환경**
  ...
  - 다만, 로컬에 GPU가 없다면 구글 Colab에서 main2.py 실행으로 이미지 재생성과 검색을 구현하실 수 있습니다. 로컬 환경에서 스케치를 그리고, 해당 스케치 경로를 image_path에 입력하면 됩니다. 또한, 갤러리를 구글 드라이브에 업로드한 뒤, database에 경로를 입력하면 사용할 수 있습니다.
